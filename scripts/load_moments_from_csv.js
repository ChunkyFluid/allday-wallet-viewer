...
function buildBatchInsert(rows) {
    ...
    const cols = ["nft_id", "edition_id", "play_id", "serial_number", "minted_at", "burned_at", "current_owner"];
    ...
    for (const r of rows) {
        ...
        params.push(r.nft_id, r.edition_id, r.play_id, r.serial_number, r.minted_at, r.burned_at, r.current_owner);
    }
...
async function loadMoments() {
    ...
    console.log("Found moments.csv, starting load (streaming + batched inserts)...");
    ...
    try {
        for await (const row of parser) {
            ...
            const serial_number = row.serial_number && row.serial_number !== "" ? Number(row.serial_number) : null;
            const minted_at = row.minted_at || null;
            const burned_at = row.burned_at || null;
            const current_owner =
                row.current_owner && String(row.current_owner).trim() !== "" ? String(row.current_owner).trim() : null;
