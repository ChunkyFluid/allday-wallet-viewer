...
async function loadEditions() {
  ...
  console.log(
    "Found editions.csv, starting load (row-by-row, headers forced to lowercase)..."
  );
  ...
  try {
    for await (const row of stream) {
      ...
      const max_mint_size = row.max_mint_size
        ? Number(row.max_mint_size)
        : null;
      ...
      try {
        ...
        failed += 1;
        console.error(
          `Row failed for edition_id=${edition_id}: ${err.code || ""} ${
      ...
      if (count % 1000 === 0) {
        console.log(
          `Upserted ${count} editions so far... (failed: ${failed})`
        );
      }
    }

    console.log(
      `✅ Done. Total editions inserted/updated: ${count}, failures: ${failed}`
    );
  } catch (err) {
